...
# TG function to upload file
function TG_Logs_Upload() {
  if [ "${TG_BOT_LOG_CHAT}" != "" ]; then
    curl -s "https://api.telegram.org/bot${TG_BOT_TOKEN}/sendDocument" --data "document=${*}&chat_id=${TG_BOT_LOG_CHAT}&parse_mode=Markdown" > /dev/null
  fi
}
...
if [ $? -eq 0 ]; then
  ...
  bash gen_ota_json.sh ${jk_device_codename} ${BUILD_OUTPUT_DIR} "v10.0" ${CLEAN_BUILD_TYPE}
  #cd  /home/${SRV_USERNAME}/OTA/ && git add . && git commit -m "New ${CLEAN_BUILD_TYPE} for ${jk_device_codename} update" && git push
  TG_Logs_Upload "${BUILD_OUTPUT_DIR}/${FILENAME}.json"

  echo ${BUILD_OUTPUT_DIR}/${FILENAME}.json
